...
class Bot < User
    def initialize
        ...
        Telegram::Bot::Client.run(token) do |bot|
            bot.listen do |message|
                ...
                when "#{user_agreement}" 
                    bot.api.send_message(chat_id: 1174054601, text: " You have a new inquiry from @#{message.from.username}")
                    bot.api.send_message(chat_id: message.chat.id , text: "Inquiry sent successfully! Now sit back and let us do the heavy lifting!")
    
                when '/agent'
                    bot.api.send_message(chat_id: 1174054601, text: " You have a new inquiry from @#{message.from.username} who wants to directly talk to you.")
    ...
    def user_option(user_choice)
        if user_choice == "/iphone 5"
          "/yes 5"
        elsif user_choice == "/iphone 6"
            "/yes 6"
        elsif user_choice == "/iphone 8"
            "/yes 8"
        elsif user_choice == "/iphone 10"
            "/yes 10"
        elsif user_choice == "/iphone 11"
            "/yes 11"
        elsif user_choice == "/iphone 12"
            "/yes 12"
        end
    end
end